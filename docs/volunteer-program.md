...
```mermaid
graph TD
    A[Recruit] --> B[BHD Basic Training]
    B --> C[Basic Responder]
    C --> D{Active 6 months<br/>5+ responses?}
    D -->|Yes| E[BHD Plus Training]
    D -->|No| F[Remains Basic]
    E --> G[Advanced Responder]
    G --> H{Active 2 years<br/>20+ responses?<br/>Leadership shown?}
    H -->|Yes| I[Instructor Training]
    H -->|No| J[Remains Advanced]
    I --> K[Certified Instructor]
    K --> L[Can teach BHD Basic]
    L --> M{Can teach<br/>BHD Plus?}
    M -->|Yes| N[Senior Instructor]
    M -->|No| O[Remains Instructor]
```
